...
type View  = 'calculator' | 'pj' | 'clt' | 'terms' | 'privacy' | 'cookies';
type Theme = 'dark' | 'light';
type Mode  = 'comparativo' | 'pj' | 'clt';
...
const App: React.FC = () => {
  ...
  const [mode, setMode] = useState<Mode | null>(() => (localStorage.getItem('calc_mode') as Mode) || null);
  const [showModeModal, setShowModeModal] = useState(() => !localStorage.getItem('calc_mode'));
  ...
  function handleModeSelect(m: Mode) {
    localStorage.setItem('calc_mode', m);
    setMode(m);
    ...
    <div style={{ background: v('bg'), color: v('t1'), minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 flex items-center justify-between" style={{ height: 72 }}>
          {/* Logo */}
          <button
            onClick={() => setView(mode === 'pj' ? 'pj' : mode === 'clt' ? 'clt' : 'calculator')}
            className="flex items-center gap-3"
          >
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: mode === 'clt' ? cltColor : pjColor }}>
            ...
            <div className="text-left">
              <p className="text-base font-bold leading-tight" style={{ fontFamily: 'Roboto, sans-serif', color: v('t1') }}>
                {mode === 'pj'
                  ? <>Calculadora <span style={{ color: pjColor }}>PJ</span></>
                  : mode === 'clt'
                    ? <>Calculadora <span style={{ color: cltColor }}>CLT</span></>
          ...
          <div className="flex items-center gap-3">
            ...
            {mode && (
              <div
                ...
                style={{ border: `1px solid ${v('border2')}`, background: v('surface2') }}
              >
                {(['comparativo', 'pj', 'clt'] as Mode[]).map(m => {
                  const active = mode === m;
                  const activeBg = m === 'clt' ? cltColor : pjColor;
                  return (
                    <button
                      key={m}
                      ...
                      style={{
                        ...
                        color: active ? (isDark ? '#0C0E14' : '#fff') : v('t3'),
                        transition: 'all 0.15s',
                      }}
                    >
                      {m === 'comparativo' ? 'CLT × PJ' : m === 'pj' ? 'PJ' : 'CLT'}
                    </button>
                  );
                })}
              </div>
            ...
            {/* Live tables badge */}
            <div
              className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-bold tracking-[0.15em] uppercase"
              style={{ background: v('surface2'), color: pjColor, border: `1px solid ${v('border2')}`, fontFamily: 'Roboto, sans-serif' }}
            >
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: pjColor, animation: 'glowPulse 2s ease-in-out infinite' }} />
              Tabelas {year}
            </div>
      ...
      {/* Cookie consent — aparece enquanto não aceitar (null ou declined) */}
      {consent !== 'accepted' && (
        <CookieConsent onAccept={handleAccept} onDecline={handleDecline} isDeclined={consent === 'declined'} />
      )}
